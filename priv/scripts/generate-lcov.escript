...
main(_) ->
    lists:foreach(
        fun cover:import/1, filelib:wildcard("_build/test/cover/*.coverdata")
    ),
    code:add_path("_build/test/lib/neo/ebin"),
    ...
    io:format("  cover details written to: ~s\n", [filename:absname(LCOVPath)]).

analyse(Module) ->
    SourceFile = proplists:get_value(source, Module:module_info(compile)),
    ...
    FunctionPositions = maps:from_list([
        {
            ...
            erl_anno:line(erl_syntax:get_pos(Function))
        }
     || Function <- Forms,
