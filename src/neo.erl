...
-spec get(#{ Key => Value }, Key) -> {ok, Value} | {error, notfound}
...
-spec get_(#{ Key => Value }, Key) -> Value | no_return()
...
-spec get(#{ Key => Value }, Key, Default) -> Value | Default
       ; ([Element], Lookup, Default) -> Value | Default
when
    Element :: {Key, Value} | Value,
    Lookup  :: Key | pos_integer(),
    Key     :: atom() | binary(),
...
-spec pop(#{ Key => Value }, Key) -> {Value, map()} | no_return()
       ; ([Element], Lookup) -> {Value, [Element]} | no_return()
when
    Element :: {Key, Value} | Value,
    Lookup  :: Key | pos_integer(),
    Key     :: atom() | binary(),
...
key_member([_Map|Maps], Key, Value) ->
    key_member(Maps, Key, Value);
...
key_store([], _Key, _Value, NewMap) -> [NewMap].
...
dget_internal({ok, Object}, [Key|Path]) ->
    dget_internal(get(Object, Key), Path).

dset_internal(_Map, [], Value) -> Value;
...
ddelete_internal(Map, [Key]) -> maps:remove(Key, Map);
...
key_sorter(Maps, Key, SorterFun) ->
    [
        ...
        case Tuple of
            {_Value, Map} -> Map;
            Unknown -> erlang:error(neo_test_helpers:format(
...
group_unique_by_test_() -> ?function_test(
    group_unique_by(Maps, Key),
    [Maps, Key],
    #{
        [[], key]
...
key_replace_test_() -> ?function_test(
...
key_delete_test_() -> ?function_test(
...
deep_map_keys_test_() -> ?function_test(
